...
#  Date Created: February 29th, 2020

#  Date Last Modified: March 2nd, 2020
...
def countHi(string):
    ...
    if string == []:
        return 0
    if ord(string[-1]) == 105 and ord(string[-2]) == 104:
        string.pop()
        return 1 + countHi(string)
    string.pop()
    return countHi(string)
...
def changePi(string):
    ...
    return string[0:1] + changePi(string[1:])
...
def array220(nums, index):
    if len(nums) < 2 or index == len(nums):
        return False
...
def countHi2(string):
    if len(string) <= 1:
        return 0
    if string[1:3] == 'hi' and string[0] != 'x':
        return countHi2(string[3:]) +1
    if string[:2] =='hi':
        return countHi2(string[2:])+1
    if string [1:3] =='hi'and string[0]=='x':
...
def strDist(string, sub):
    ...
    if string [:len(sub)] == sub and string[-len(sub):] == sub:
        return len(string)
    if string[:len(sub)] == sub:
        return strDist(string[:len(string)-1],sub)
    if string[-len(sub):len(string)-1] == sub:
